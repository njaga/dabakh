...
$req_vente=$db->prepare("SELECT CONCAT(client.prenom,' ', client.nom), vente.date_vente, vente.montant, client.id, vente.frais_transport
FROM `vente`
INNER JOIN client ON client.id=vente.id_client
WHERE vente.id=?");
$req_vente->execute(array($_GET['id']));
$donnee_vente=$req_vente->fetch();
$client=$donnee_vente['0'];
$date_vente=$donnee_vente['1'];
$montant=$donnee_vente['2'];
$id_client=$donnee_vente['3'];
$frais_transport=$donnee_vente['4'];
	...
	<body style="background-image: url(<?= $image ?>bgaccueil.jpg);">
		...
		<div class="container white">
			<div class="row z-depth-5" >
				...
				<form class="col s10 offset-s1 center" method="POST" id="form" action="m_vente_trmnt.php" >
                <input type="number" name="id_vente"  value="<?=$_GET['id'] ?>" hidden>
                <input type="number" name="id_client"  value="<?=$id_client ?>" hidden>
                    <div class="row">
                        <h3 class="center ">Client : <?= $client ?></h3>
                    </div>
                    <div class="row">
                        <h4 class="col s12">Liste des produits</h4>
                    ...
                    <div class="row">
                        <div class="col s8 m4 l4 input-field">
                            <input type="text" name="date_vente" id="date_vente" value="<?= $date_vente ?>" class="datepicker date_vente" required>
                            <label for="date_vente">Date vente</label>
                        </div>
                        <div class="col s8 m4 l4 input-field">
                            <input type="text" name="frais_transport" id="frais_transport" value="<?= $frais_transport ?>" required>
                            <label for="frais_transport">Frais de transport</label>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col s2 m2 input-field">
                            <a onclick="window.history.go(-1)" class="btn red">Annuler</a>
                        </div>
