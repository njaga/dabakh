...
$req_annee=$db->query('SELECT DISTINCT YEAR(date_depense) FROM cotisation_locataire_depense');
...
<html>
	<head>
		<title>Dépense sur cotisaition</title>
	...
	<body id="debut" style="background-image: url(<?= $image ?>bgaccueil.jpg);">
		...
		<div class="row">
			<h4 class="center #0d47a1 col s12" style="color: white">Dépenses sur cotisation pendant le mois de :</h4>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$('.tooltipped').tooltip();
			function l_cotisation_depense(mois, annee,search)
			{
				$.ajax({
				type:'POST',
				url:'l_cotisation_locataire_depense_ajax.php',
			...
			l_cotisation_depense(mois, annee,search);
