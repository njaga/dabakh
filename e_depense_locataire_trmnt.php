...
$req=$db->query('SELECT MAX(id) FROM depense_locataire');
$donnees= $req->fetch();

$id_cotisation_locataire=$donnees['0']+1;
$req=$db->prepare('INSERT INTO depense_locataire(id, motif, mois, annee, type_depense, date_depense, montant, id_locataire, id_user) values (?,?, ?, ?, ?, ?, ?, ?, ?) ');
