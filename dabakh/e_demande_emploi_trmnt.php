...
$req=$db->query("SELECT MAX(id) FROM `demande_emploi`");
$donnees=$req->fetch();
...
$req=$db->prepare('INSERT INTO demande_emploi (id, date_enregistrement, prenom, nom, poste, id_user) VALUES (?,?,?,?,?,?)');
$nbr=$req->execute(array($id_demande, $date_enregistrement, $prenom, $nom, $poste, $id_user)) or die(print_r($req->errorInfo()));
