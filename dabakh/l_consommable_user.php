...
$annee= date('Y');
$req_annee=$db->query('SELECT DISTINCT YEAR(date_operation) FROM `consommable_utilisation`');
...
<html>
	<head>
		...
		<title>Liste dotation consommables</title>
	</head>
			...
			<h4 class="center #0d47a1 col s12" style="color: white">Ravitaillement effectués pendant le mois de :</h4>
			 <?php
		      if (($_SESSION['fonction']=='medecin') OR ($_SESSION['fonction']=='administrateur') OR ($_SESSION['fonction']=='secretaire'))
		      {
		        ?>
		      <li>
		        <a href="e_consommable_user.php" class="btn col s12 m2">Nouvelle dotation</a>
		      </li>
		      <?php
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$('.tooltipped').tooltip();
			function l_consommable_user_ajax(mois, annee,search)
			{
				$.ajax({
				type:'POST',
				url:'l_consommable_user_ajax.php',
			...
			l_consommable_user_ajax(mois, annee,search);
