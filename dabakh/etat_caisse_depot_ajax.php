...
if ($mois==1) 
{
	...
	$reponse=$db->prepare("SELECT (SELECT COALESCE(SUM(montant),0) FROM caisse_depot WHERE type='entree' AND date_operation<?)  - (SELECT COALESCE(SUM(montant),0) FROM caisse_depot WHERE type='sortie' AND  date_operation<?)");
...
if ($nbr>0) 
{
	while ($donnees= $reponse->fetch())
	{
		...
		echo "<td> <a class='tooltipped' data-position='top' data-delay='50' data-tooltip='cliquez ici pour modifier' href='m_caisse_depot.php?id=$id'> ".$date_operation. "</a> </td>";
		}
		else
		{
