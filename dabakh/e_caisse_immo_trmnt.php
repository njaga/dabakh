...
if ($nbr>0) 
{
	...
	$nbr=$req->execute(array($type, $section, $motif, $num_cheque, $montant, $date_operation, $_SESSION['service'], $pj,  $id_user)) or die(print_r($req->errorInfo()));
	}
	if ($section=="Versement") 
	{
		...
		$type="entree";
		$section="Versement";
		$motif=strtoupper("Versement banque / caisse immo");
		$req=$db->prepare('INSERT INTO banque(type, section, motif, num_cheque, montant,date_operation, structure, pj, id_user) values (?,?,?,?,?,?,?,?,?) ');
	$nbr=$req->execute(array($type, $section, $motif, $num_cheque, $montant, $date_operation, $_SESSION['service'], $pj, $id_user)) or die(print_r($req->errorInfo()));
	}
...
for ($i=0; $i < $nbr_pj; $i++)
{
	...
	default:
	{
		...
		{
			$repertoire='../pj/caisse_immo/'.$id_caisse.'/';
