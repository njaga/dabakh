...
if ($search=="") 
{
	$reponse=$db->query("SELECT * FROM `bailleur` WHERE etat='desactiver'
	ORDER BY annee_inscription DESC, num_dossier DESC");
}
else
{
	$reponse=$db->prepare("SELECT * FROM `bailleur` WHERE CONCAT (prenom,' ',nom) like CONCAT('%', ?, '%') AND etat='desactiver' ORDER BY annee_inscription DESC, num_dossier DESC");
