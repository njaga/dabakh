...
$req_annee=$db->query('SELECT DISTINCT YEAR(date_ravitaillement) FROM `ravitaillement_produit`');
...
<html>
			...
			<h4 class="center #0d47a1 col s12" style="color: white">Ravitaillement effectués pendant le mois de :</h4>
			 <?php
		      if (($_SESSION['fonction']=='medecin') OR ($_SESSION['fonction']=='administrateur') OR ($_SESSION['fonction']=='secretaire'))
		      {
		        ?>
		      <li>
		        <a href="e_ravitaillement_produit.php" class="btn col s12 m2">Nouveau ravitaillement</a>
		      </li>
		      <?php
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$('.tooltipped').tooltip();
			function l_ravitaillement_produit_ajax(mois, annee,search)
			{
				$.ajax({
				type:'POST',
				url:'l_ravitaillement_produit_ajax.php',
			...
			l_ravitaillement_produit_ajax(mois, annee,search);
