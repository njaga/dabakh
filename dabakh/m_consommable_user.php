...
$req=$db->prepare("SELECT consommable_utilisation.id, consommable_utilisation.date_operation, consommable.consommable, consommable_utilisation.quantite, consommable_utilisation.demandeur, consommable_utilisation.commentaire, consommable_utilisation.id_user, consommable.qt 
FROM `consommable_utilisation`, consommable 
WHERE consommable_utilisation.id_consommable=?");
	...
	<body style="background-image: url(<?= $image ?>bgaccueil.jpg);">
		...
		<div class="container white">
			<div class="row z-depth-5" style="padding: 10px;">
				<h3 class="center">Modification opération dotation</h3>
				...
				<form class="col s12" method="POST" id="form" action="m_consommable_user1_trmnt.php?id=<?=$_GET['id']?>" >
					<div class="row">
						<div class="col s5 input-field">
							<input type="text" value="<?=$date_operation ?>"  name="date_operation" class="datepicker" id="date_operation" required>
							<label for="date_operation">Date opération</label>
						</div>
						<div class="col s3 input-field">
							<input type="number" value="<?=$quantite ?>" name="qt" id="qt" required>
							<label for="qt">Quantité</label>
						</div>
					</div>
					<div class="row">
						<div class="col s4 input-field">
							<input type="text" value="<?=$demandeur ?>"  name="demandeur"  id="demandeur" required>
							<label for="demandeur">Demandeur</label>
						</div>
					</div>
					<div class="row">
						<div class="col s7 input-field">
							<textarea class="materialize-textarea" id="commentaire" name="commentaire"><?=$commentaire ?></textarea>
